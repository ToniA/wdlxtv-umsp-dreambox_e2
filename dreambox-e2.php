<?php
    ...
    function _pluginMain($prmQuery) {
      ...
      if ($queryData['bouquet'] != '') {
        return Dreambox_E2_Channels($queryData);
      }
      elseif ($queryData['recordingspath'] != '') {
        return Dreambox_E2_Recordings($queryData);
      } else {
          $myMediaItems[] = array(
    ...
    function Dreambox_E2_Recordings($queryData) {
    ...
    function Dreambox_E2_Channels($queryData) {
      ...
      if ( $bouquet == '' ) {
        ...
        while ($reader->read()) {
          if (($reader->nodeType == XMLReader::ELEMENT) && ($reader->localName == 'e2service')) {
            ...
            $retMediaItems[] = array (
              'id'             => $newChannel['id'],
    # Requires my custom streamproxy binary on the Dreambox
    #          'res'            => 'http://' . $dreamboxAddress . ':8001/' . $newChannel['id'],
